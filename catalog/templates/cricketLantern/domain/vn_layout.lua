...
M.LINE_PITCH = 30
-- The 188px dialogue slab reserves one title row plus four 20px text rows.
-- Four lines make full use of the e-ink panel without reducing the fixed face.
M.LINES_PER_PAGE = 4
M.CHOICE_SLOT = 62
M.GLYPH_H = 24
M.EDGE = 12
...
function M.compute(ctx, visible_choices)
  ...
  -- The old CSS-style cover calculation started a 560px bitmap at x=-40.
  -- Its crop is now baked into 480×600 art, so this command fills the stage
  -- while remaining wholly inside the physical device frame.
  ...
  -- Always top-align the text block to this first body row. Pages can end
  -- short, so vertical centering would create a false blank line.
...
-- A completed route gets a dedicated, unambiguous restart affordance instead
-- of reusing the ordinary floating-choice layout.
