...
local function assert_story(story, max_choices)
  ...
  for id, node in pairs(story.nodes) do
    ...
    if node.ending ~= nil then
      ...
      -- The dedicated result view deliberately has one clear affordance. It
      -- restarts through the normal choice/effect path, so no game-specific
      -- navigation is baked into the runtime.
      assert(type(node.choices) == "table" and #node.choices == 1, label .. ".ending requires exactly one restart choice")
      assert(type(node.choices[1]) == "table" and node.choices[1].next == RESTART, label .. ".ending choice must restart")
      local ending = node.ending
      assert(type(ending.name) == "string" and ending.name ~= "", label .. ".ending.name required")
      assert_text_fits(ending.name, Layout.MAX_LINE_UNITS, label .. ".ending.name")
      if ending.image ~= nil then assert(type(ending.image) == "string" and ending.image ~= "", label .. ".ending.image must be a non-empty string") end
      if ending.summary ~= nil then
        assert_string_list(ending.summary, label .. ".ending.summary")
        for index, line in ipairs(ending.summary) do assert_text_fits(line, Layout.MAX_LINE_UNITS, label .. ".ending.summary[" .. tostring(index) .. "]") end
      end
      if ending.notes ~= nil then
        assert(type(ending.notes) == "table", label .. ".ending.notes must be a table")
        for index, note in ipairs(ending.notes) do
...
function M.bind(story, opts)
  ...
  local function snapshot_of(s)
    return {
      ...
      choices_revealed = s.choices_revealed,
  ...
  local function refresh_ui(s)
    sync_reading(s)
    s.ui_choice_count = s.reading_done and s.choices_revealed and #choices_of(s) or 0
  end
  ...
  function E.ensure(ctx)
    ...
    s.choices_revealed = s.choices_revealed == true
  ...
  function E.advance(s)
    ...
    if s.reading_done then
      local cs = choices_of(s)
      if #cs > 0 and not s.choices_revealed then
        s.choices_revealed = true
        refresh_ui(s)
        return true
      end
      return false
  ...
  function E.status_items(s)
    ...
    for _, k in ipairs(keys) do
      items[#items + 1] = { key = k, label = tostring(labels[k] or k), value = tostring(s.vars[k] or 0), meaning = tostring(meanings[k] or "") }
    end
    return items
  end
  ...
  function E.ending_notes(s)
    local n = node_of(s)
    local out = {}
    for _, note in ipairs((n and n.ending and n.ending.notes) or {}) do
  ...
  function E.restore_checkpoint(s, index)
    ...
    s.choices_revealed = snapshot.choices_revealed == true
