...
function M.draw_stroke_time(g,text,x,y,width,height,color,variant)
  local digits,colons=0,0; for i=1,#text do if text:sub(i,i)==":" then colons=colons+1 else digits=digits+1 end end
  local gap=math.max(8,math.floor(width*.022)); local colon_w=math.max(20,math.floor(width*.03)); local digit_w=math.floor((width-colons*colon_w-(digits-1)*gap)/digits); local digit_h=math.min(height,math.floor(digit_w*1.56)); local total=digits*digit_w+(digits-1)*gap+colons*colon_w; local cursor=x+math.floor((width-total)/2); local top=y+math.floor((height-digit_h)/2)
  ...
  for i=1,#text do local ch=text:sub(i,i)
    if ch==":" then local dot=variant=="tube" and 9 or math.max(3,radius-6); g:circle(cursor+math.floor(colon_w/2),top+math.floor(digit_h*.36),dot,"fill",color); g:circle(cursor+math.floor(colon_w/2),top+math.floor(digit_h*.68),dot,"fill",color); if variant=="tube" then g:circle(cursor+math.floor(colon_w/2),top+math.floor(digit_h*.36),math.max(2,dot-3),"fill",M.WHITE); g:circle(cursor+math.floor(colon_w/2),top+math.floor(digit_h*.68),math.max(2,dot-3),"fill",M.WHITE) end; cursor=cursor+colon_w
...
function M.draw_stroke_time_rot(g,text,cx,cy,width,height,deg,color,variant)
  ...
  for i=1,#text do local ch=text:sub(i,i)
    if ch==":" then
      ...
      if variant=="tube" then g:circle(dx,dy,math.max(2,dot-3),"fill",M.WHITE); g:circle(ex,ey,math.max(2,dot-3),"fill",M.WHITE) end
