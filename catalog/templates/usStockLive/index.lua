-- 美股实况：通过 ScriptNet GET 固定 Demo API，查询真实美股报价。
...
local API_BASE = "http://193.112.174.92:28473/demo/market/detail/xtapp?kind=us&symbol="
local KLINE_API_BASE = "http://193.112.174.92:28473/demo/market/detail/kline/xtapp?kind=us&limit=40&type="
local RANKING_API = "http://193.112.174.92:28473/demo/market/us/ranking/xtapp?limit=12"
local QUOTE_PROTOCOL = "XTAPP_STOCK_V1"
local KLINE_PROTOCOL = "XTAPP_KLINE_V1"
local RANKING_PROTOCOL = "XTAPP_RANKING_V1"
local DEFAULT_CODE = "AAPL"
local PRESETS = {
  { code = "AAPL", label = "苹果" },
  { code = "MSFT", label = "微软" },
  { code = "NVDA", label = "英伟达" },
...
local function app_state(ctx)
  ctx.state.us_stock_live = ctx.state.us_stock_live or {
    code = DEFAULT_CODE,
    cursor = 3,
    editor_open = false,
    status = "idle",
  ...
  local state = ctx.state.us_stock_live
  if state.default_code_version ~= "us_default_v1" then
    state.code = DEFAULT_CODE
    state.default_code_version = "us_default_v1"
  end
  if type(state.code) ~= "string" or not state.code:match("^[A-Z.][A-Z.][A-Z.]*$") or #state.code > 8 then state.code = DEFAULT_CODE end
...
local function draw_ranking(g, state)
  g:text(24, 168, "美股涨幅榜", { color = BLACK })
...
local function draw_quote(g, state)
  ...
  if not quote then
    local title = state.status == "loading" and "LOADING" or "US STOCK LIVE"
  ...
  g:text(24, 154, quote.name ~= "" and quote.name or quote.enname or "美股", { color = BLACK })
...
local function draw_code_modal(g, state)
  ...
  center_text(g, 16, 204, 448, "输入美股代码（最多 5 位）", BLACK)
...
function on_draw(ctx, g)
  local state = app_state(ctx)
  g:clear(WHITE)
  g:text(24, 25, "美股实况", { color = BLACK })
  right_text(g, 456, 25, "LIVE · US")
